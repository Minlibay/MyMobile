...
interface StepsDao {
    ...
    suspend fun getInRange(userId: Long, start: Int, end: Int): List<com.volovod.alta.data.model.DayValue>

    @Query("SELECT dateEpochDay, steps as value FROM step_entries WHERE userId = :userId AND dateEpochDay >= :start AND dateEpochDay <= :end")
    fun observeInRange(userId: Long, start: Int, end: Int): Flow<List<com.volovod.alta.data.model.DayValue>>
