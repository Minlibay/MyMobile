...
import com.volovod.alta.data.repository.SmokeRemoteRepository
...
fun HealthScreen() {
    ...
    val authRepository = remember(context, sessionStore, db) { 
        com.volovod.alta.data.repository.AuthRepository(
            ...
            db.profileDao(),
            db.userSettingsDao(),
            com.volovod.alta.data.repository.SyncRepository(sessionStore, db),
        )
    }
        ...
        weightRemoteRepository.getWeightRange(start = today - 60, end = today).fold(
            onSuccess = { rows ->
                withContext(Dispatchers.IO) {
                    ...
                    rows.forEach { r ->
                        db.weightDao().upsert(
                            com.volovod.alta.data.entities.WeightEntryEntity(
                                userId = userId,
